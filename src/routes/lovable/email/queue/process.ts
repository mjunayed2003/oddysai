  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const expectedBuf = Buffer.from(supabaseServiceKey)
        if (
          ...
          if (messageIds.length > 0) {
            ...
            if (failedRowsError) {
              ...
              for (const row of failedRows ?? []) {
                ...
                if (/429|rate_limited|high demand/i.test(errorMessage)) continue
            ...
            try {
              ...
              if (isRateLimited(error)) {
                await supabase.from('email_send_log').insert({
                  ...
                  recipient_email: payload.to,
                  status: 'pending',
                  error_message: `Deferred due to temporary email rate limit: ${errorMsg}`.slice(0, 1000),
                })
